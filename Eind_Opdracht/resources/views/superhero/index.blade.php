    ...
    <div class="max-w-4xl mx-auto mt-8">
        ...
        <div class="mb-4">
            <h1 class="text-3xl font-bold text-center">
                Laravel 9 CRUD With Tailwind CSS Superheroes
            </h1>
        </div>

        <div class="flex justify-end mt-10">
            <a href="{{ route('superhero.create') }}" class="px-2 py-1 rounded-md bg-blue-500 text-sky-100 hover:bg-blue-700">+ Create New Superhero</a>
        </div>
                    ...
                    @endif
                     ...
                     <table class="min-w-full">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-xs font-medium leading-4 tracking-wider text-left text-gray-500 uppercase border-b border-gray-200 bg-gray-50">Publisher</th>
                        ...
                        <tbody class="bg-white">
                            @foreach ($superhero as $superhero)
                            <tr>
                                <td class="px-6 whitespace-no-wrap border-b border-gray-200">{{ ++$i }}</td>
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">{{ $superhero->name }}</td>
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">{{ $superhero->publisher }}</td>
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200"><img src="/images/{{ $superhero->image }}" width="100px"></td>
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                    <form action="{{ route('superhero.destroy',$superhero->id) }}" method="POST">
                        
                                        <a class="text-indigo-600 hover:text-indigo-900" href="{{ route('superhero.show',$superhero->id) }}">
                                        ...
                                        <a href="{{ route('superhero.edit',$superhero->id) }}" class="text-indigo-600 hover:text-indigo-900">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 inline-block" fill="none" viewBox="0 0 24 24" stroke="currentColor">
